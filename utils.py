...
def calc_predicted_clusters(coref_logits, mention_logits, coref_threshold, gold_mentions: List, slots=False):
    ...
    if gold_mentions is None:
        ...
        for i in range(bs):
            ...
            if slots:
                max_coref_score, max_coref_cluster_ind = cur_coref_logits[0].max(-2) #[gold_mention] choosing the index of the best cluster per gold mention
            ...
            cluster_id_to_tokens = {k: list(v) for k, v in itertools.groupby(sorted(list(zip(true_coref_indices, \
                max_coref_cluster_ind_filtered.numpy())), key=lambda x: x[-1]), lambda x: x[-1])}
...
def calc_best_avg_f1(all_coref_logits, all_mention_logits, all_gold_clusters, \
    all_mentions, coref_threshold, thresh_delta, slots):
    best = [-1, -1, -1, -1, -1, -1]
    best_metrics = []
    ...
    for coref_threshold in tqdm(np.arange(thresh_coref_start, thresh_coref_end, thresh_delta), desc='Searching for best threshold'):
        p, r, f1, pm, rm, f1m, pmp, rmp, f1mp, metrics = evaluate_by_threshold(\
            all_coref_logits, all_mention_logits, all_gold_clusters, \
                coref_threshold, all_mentions, slots)
        if f1 > best[-1]:
            best = pmp, rmp, f1mp, pm, rm, f1m, p,r,f1
            best_metrics = metrics
            best_coref_threshold = coref_threshold

    return best + (best_coref_threshold,) + (best_metrics,)

def evaluate_by_threshold(all_coref_logits, all_mention_logits, all_gold_clusters, coref_threshold, all_mentions, slots):
    ...
    for i, (coref_logits, gold_clusters, mentions) in enumerate(
        ...
        if len(all_mention_logits) > 0:
            ...
            predicted_clusters = calc_predicted_clusters(coref_logits.unsqueeze(0), mention_logits.unsqueeze(0), coref_threshold, [mentions], slots)
        else:
            predicted_clusters = calc_predicted_clusters(coref_logits.unsqueeze(0), [], coref_threshold, [mentions], slots)

...
class Adi(BertPreTrainedModel):
    ...
    #     outputs = self.longformer(input_ids, attention_mask=attention_mask)  # Getting representation for each token in the text
    #     sequence_output = outputs[0]  # [batch_size, seq_len, dim]
    #
    #     # Compute representations
    #     start_mention_reps = self.start_mention_mlp(sequence_output) if self.do_mlps else sequence_output  # noted as "m^s" at section 3
    #     end_mention_reps = self.end_mention_mlp(sequence_output) if self.do_mlps else sequence_output # noted as "m^e" at section 3
    #
    #     start_coref_reps = self.start_coref_mlp(sequence_output) if self.do_mlps else sequence_output # noted as "a^s" at section 3
    #     end_coref_reps = self.end_coref_mlp(sequence_output) if self.do_mlps else sequence_output # noted as "a^e" at section 3
    #
    #     # mention scores
    #     mention_logits = self._calc_mention_logits(start_mention_reps, end_mention_reps) # noted as "f_m(q)" at section 3
    #
    #     # prune mentions
    ...
    #     coref_logits = self._calc_coref_logits(topk_start_coref_reps, topk_end_coref_reps) # noted as "f_a(c,q)" at section 3
    #

...
def train(args, model, criterion, train_loader, eval_loader, eval_dataset):
    ...
    param_dicts = [
        {"params": [p for n, p in model.named_parameters() if "longformer" not in n and p.requires_grad]},
        {
            "params": [p for n, p in model.named_parameters() if "longformer" in n and p.requires_grad],

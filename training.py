...
def train(args, model, criterion, train_loader, eval_loader, eval_dataset):
    ...
    for epoch in train_iterator:
        ...
        if args.local_rank in [-1, 0]:
            ...
            if traineval_epochs > 0 and (epoch + 1) % traineval_epochs == 0:
...
def eval_train(train_dataloader, eval_dataset, args, model):
    ...
    for batch in tqdm(train_dataloader, desc="Evaluating Train"):
        ...
        with torch.no_grad():
            ...
            predicted_clusters = calc_predicted_clusters(coref_logits.cpu().detach(), mentions_list, args.num_queries)
    ...
    print_predictions(all_coref_logits_cuda, all_gold_clusters, all_gold_mentions, all_input_ids, args, eval_dataset.tokenizer)
